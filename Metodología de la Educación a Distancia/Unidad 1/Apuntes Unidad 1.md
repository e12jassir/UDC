...
```mermaid
graph LR
    A["1ª Gen: Correspondencia<br/>(1840 - Siglo XIX)"] --> B["2ª Gen: Telecomunicaciones<br/>(Mid Siglo XX)"]
    B --> C["3ª Gen: Telemática<br/>(Años 80 / 90)"]
    C --> D["4ª Gen: Campus Virtual<br/>(LMS / Moodle)"]
    D --> E["5ª Gen: Web 2.0 & IA<br/>(B-Learning / Flexibilidad)"]
```
...
```mermaid
flowchart TD
    DEF["DEFINICIONES Y MARCOS TEÓRICOS DE LA EAD"]
    DEF --> ARETIO["Lorenzo García Aretio (1999, 2004)<br/>Sistema tecnológico bidireccional<br/>Modelo Integrador B-Learning"]
    DEF --> USDLA["USDLA & ACTE (1999)<br/>Experiencia planificada<br/>Énfasis en Mediación Tecnológica"]
    DEF --> ACESAD["ACESAD (2013)<br/>Sistema Educativo Abierto<br/>Autogestión & Contexto Social"]
```
...
```mermaid
graph TD
    subgraph TRAD["Modelo Tradicional Presencial"]
        T1["Docente = Emisor Único"]
        T2["Alumno = Receptor Pasivo"]
        T3["Espacio = Aula Cerrada"]
        T4["Tiempo = Horario Rígido"]
    end
    subgraph MEAD["Modelo EAD UDC (MEAD)"]
        M1["Tutor = Mediador / Diseñador"]
        M2["Alumno = Aprendiente Autónomo"]
        M3["Espacio = Ambiente Abierto / SIMA"]
        M4["Tiempo = Autorregulado (TAE/ATS/ATA)"]
    end
```
...
```mermaid
graph TD
    V["VALORES UNIVERSALES DE LA EAD (García Aretio)"]
    V --> V1["Autonomía"]
    V --> V2["Compromiso"]
    V --> V3["Colaboración & Trabajo en Equipo"]
    V --> V4["Responsabilidad"]
    V --> V5["Solidaridad"]
    V --> V6["Diálogo & Tolerancia"]
    V --> V7["Honestidad Académica"]
```
...
sequenceDiagram
    autonumber
    participant Estudiante as Momento 1: Estudio Autónomo (TAE)
    participant CIPAS as Momento 2: Trabajo Colaborativo (CIPAS)
    participant Tutor as Momento 3: Tutoría Sincrónica (ATS)

    Estudiante->>Estudiante: Lee el módulo PDF y redacta el Protocolo Individual
    Estudiante->>CIPAS: Lleva su protocolo individual al grupo CIPAS
    CIPAS->>CIPAS: Debaten, contrastan ideas y construyen el Protocolo Colaborativo
    CIPAS->>Tutor: Presentan dudas complejas y protocolos en la Tutoría del Sábado
    Tutor->>Estudiante: Resuelve vacíos, lidera el debate y consolida el saber
```
...
<details>
<summary><b>1. ¿Por qué la EAD según ACESAD (2013) se considera un "sistema educativo abierto"?</b></summary>
...
<details>
<summary><b>2. ¿En qué consiste el cambio de rol del profesor a "tutor mediador" según García Aretio?</b></summary>
...
<details>
<summary><b>3. ¿Cuál es la diferencia sustancial entre E-learning y B-learning, y por qué la UDC utiliza B-learning?</b></summary>
...
<details>
<summary><b>4. ¿Qué significan y cómo se distribuyen las 96 horas totales de la asignatura MEAD?</b></summary>
